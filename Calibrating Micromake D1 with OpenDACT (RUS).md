...
Утилиту OpenDACT нужно брать тут - http://forum.seemecnc.com/viewtopic.php?f=36&t=8698 Вам нужна последняя версия `3.0.1A`. Скачайте дистрибутив, разверните его в удобную папку. Сначала запустите `setup.exe` и только потом - `Delta Kinematics Calibration Tool.exe`.
...
Я рекомендую перед калибровкой привести все значения к "заводским" настройкам, чтобы утилита начинала с "чистого листа". 

### Самый простой способ отредактировать настройки - через CURA

Запустите CURA и подключитесь к принтеру. Откройте меню `Machine` -> `Firmware configuration`. У вас откроется такое окно:

![CURA EEPROM dialog](https://pp.userapi.com/c836128/v836128745/415b2/MxaS8MbzS88.jpg)

Записывайте на бумажку старые значения, чтобы можно было вернуть их обратно, если захочется.

Вот какие "дефолтные" значения нам понадобятся:

| Настройка | Что означает | Значение |
| --- | --- | --- |
| Z max length [mm] | Высота печати | Не меняйте |
...
### Способ редактирования настроек "для продвинутых" - с помощью пачки команд gcode 

Эти команды G-code приводят настройки к исходным.
...
_(Обратите внимание, скриншоты в этой инструкции сняты со старой версии программы. Ваша версия, '3.1.0A', будет немного отличаться внешним видом.)_
...
Когда настройки сделаны, жмите `H.A.I. Calibrate` (не `A.I. Calibrate`) или же `Calibrate`. Всё остальное утилита сделает за вас.
...
Ваша задача - опустить сопло так, чтобы бумажку слегка прижало соплом - но чтобы её можно было легко протащить пальцами без усилия. Как только вы нащупали нужную точку, выберите в меню пункт `Set Z=0` - и принтер запишет в память новую правильную высоту.
